...
double scale_factor =0.002967;
double x_central = 0.543;
double y_central = 0.321;
int i_central = 319;
int j_central = 239;
double theta = 0.205048;
...
void ImageConverter::imageCb(const sensor_msgs::ImageConstPtr& msg){
	...
	//Code added for ps7
	//Calculate u and v
	double u = (i_centroid - i_central) * scale_factor;
	double v = (j_centroid - j_central) * scale_factor;

	//Translate block coordinates into robot coordinates
	block_pose_.pose.position.x = cos(theta) * u - sin(theta) * v + x_central;
	block_pose_.pose.position.y = sin(theta) * u - cos(theta) * v + y_central;
	
	//Print out x and y positions to check
