...
void callback(geometry_msgs::PoseStamped block_pose) {
    x_position = block_pose.pose.position.x;
    y_position = block_pose.pose.position.y;
    z_position = block_pose.pose.position.z;
...
int main(int argc, char** argv) {
    ...
    //Subscribe to find_block
    ros::Subscriber sub = nh.subscribe<geometry_msgs::PoseStamped>("block_pose", 1, callback);
	...
	//Wait until we get the position from find_block!
	while (!position_received) {
        ...
        tool_pose.pose.position.x = x_position; //Use position from callback
        tool_pose.pose.position.y = y_position; //Use position from callback 
	...
	ROS_INFO("requesting plan to go to desired location:");
